...
namespace NP_Archives.bsc.csit.CSIT_2_sem
{
    ...
    {
        private string pdfUrl = "https://drive.google.com/uc?export=view&id=1CxQZXnd3jrd24IWtIad3eJZ9g_L-yJ35";
        private string pdfFileName = "2semdiscretestructuretextbook"; // The name you want to save the PDF file with
        private string pdfFolderPath = Path.Combine(Application.StartupPath, "DownloadedPDFs");
        private string localFilePath;
        private string engpdfUrl = "https://drive.google.com/uc?export=view&id=1w-HYDNhrX3a_bZjmUrJkbBdfILgFIwJA";
        private string engpdfquestion = "2semdiscretestructureexamquestion"; //
        private string localfilepaths;
        private string pdfurlpart3 = "https://drive.google.com/uc?export=view&id=1f5xJCwxZiHSbTDNwW_iu4jFY38nmGwvu";
        private string pdffilname = "2semdiscretestructuremodelquestionsoln"; //
        ...
        {
            await initizated();
            String openPDFFile = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + @"Discrete Structure Syllabus.pdf";//PDF DOc name
            System.IO.File.WriteAllBytes(openPDFFile, Properties.Resources.Discrete_Structure_Syllabus);///the resource automatically creates            
            webView21.CoreWebView2.Navigate(openPDFFile);
            /*
            string opennotespdffile = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + @"Discrete Structure Note.pdf";
          //  System.IO.File.WriteAllBytes(opennotespdffile, Properties.Resources.Discrete_Structure_Note);///the resource automatically creates            
            webView22.CoreWebView2.Navigate(opennotespdffile);
            string openexampdffile = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + @"3rd sem discrete struct exam.pdf";
           // System.IO.File.WriteAllBytes(openexampdffile, Properties.Resources._3rd_sem_discrete_struct_exam);///the resource automatically creates            
            webView23.CoreWebView2.Navigate(openexampdffile);
            string openpdflabsheet = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + @"Discrete Structure Model Question Solution.pdf";
           // System.IO.File.WriteAllBytes(openpdflabsheet, Properties.Resources.Discrete_Structure_Model_Question_Solution);///the resource automatically creates            
        ...
        private async void Loadmodelquestion()
        {
            ...
            if (File.Exists(localfilepaths))
            {
                try
                {
                    ...
                    // Load the PDF file into the WebView control.
                    webView23.CoreWebView2.Navigate(localfilepaths);
                    // You can also use the Url property if you have a direct URL to the PDF.
                }
                catch (Exception ex)
                {
