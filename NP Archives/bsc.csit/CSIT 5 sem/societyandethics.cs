...
{
    public partial class societyandethics : Form
    {
        private string pdfUrl = "https://drive.google.com/uc?export=view&id=1hHQYfwVp0eHoUlab2ez6ntSZ0gPljeMS";
        private string pdfFileName = "csit5semsocietyandethicstextbook"; // The name you want to save the PDF file with
        private string pdfFolderPath = Path.Combine(Application.StartupPath, "DownloadedPDFs");
        private string localFilePath;
        private string engpdfUrl = "https://drive.google.com/uc?export=view&id=1nIwKco9678nfjJo4C0IT96NkgjX7Rtby";
        private string engpdfquestion = "csit5semsocietyandethicsexamquestion"; //
        private string localfilepaths;
        public societyandethics()
        {
            InitializeComponent();
        ...
        {
            await initizated();
            String openPDFFile = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + @"TOC Syallabus.pdf";//PDF DOc name
            System.IO.File.WriteAllBytes(openPDFFile, Properties.Resources.TOC_Syallabus);///the resource automatically creates            
            webView21.CoreWebView2.Navigate(openPDFFile);
            //webView21.CoreWebView2.Navigate("https://drive.google.com/uc?export=view&id=1FfbwLRVN_pNRly1iHhaevS9DzTEYXf_F");

        }

        private void societyandethics_Load(object sender, EventArgs e)
        {
            InitBrowser();
            LoadSettings();
            Loadmodelquestion();
