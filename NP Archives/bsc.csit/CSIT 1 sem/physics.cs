...
namespace NP_Archives.bsc.csit.CSIT_1_sem
{
    ...
    {
        private string pdfUrl = "https://drive.google.com/uc?export=view&id=1I7W0IaTjnX40rSCuEj9bfOqbxnwQnHxp";
        private string pdfFileName = "1semphysicsnotes"; // The name you want to save the PDF file with
        private string pdfFolderPath = Path.Combine(Application.StartupPath, "DownloadedPDFs");
        private string localFilePath;
        private string engpdfUrl = "https://drive.google.com/uc?export=view&id=1dKmeOYpp2rN0SFx_0vS4NWkqe1lyXBer";
        ...
        private async void InitBrowser()
        {
            ...
            //System.IO.File.WriteAllBytes(opennotespdffile, Properties.Resources._1sem_physics_notes);///the resource automatically creates            
            webView22.CoreWebView2.Navigate(opennotespdffile);
            string openexampdffile = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + @"physics 1sem exam question.pdf";
           // System.IO.File.WriteAllBytes(openexampdffile, Properties.Resources.physics_1sem_exam_question);///the resource automatically creates            
        ...
        private async void Loadmodelquestion()
        {
            ...
            if (File.Exists(localfilepaths))
            {
                try
                {
                    ...
                    var env = await Microsoft.Web.WebView2.Core.CoreWebView2Environment.CreateAsync(userDataFolder: Path.Combine(Path.GetTempPath(), $"{Environment.UserName}", subFolder, pdfFolderPath));
                }
                catch (Exception ex)
                {
        ...
        private async void downloadexmquestion()
        {
            try
            {
                using (HttpClient client = new HttpClient())
